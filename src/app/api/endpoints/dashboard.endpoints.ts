...
export const dashboardEndpoints = {
  ...
  getTokenPriceHistory: async (): Promise<TokenHistory[]> => {
    const response = await api.get<TokenHistory[]>('/api/dashboard/token/price-history');
    console.log('Token History Response:', response.data);
    return response.data;
  },
  ...
  getWalletInfo: async (): Promise<WalletInfo> => {
    const response = await api.get<WalletInfo>('/api/dashboard/user/wallet');
    return response.data;
  }
}; 